...
function initialize(data){var myOptions={zoom:5,center:new google.maps.LatLng(0,0),disableDefaultUI:true,scrollwheel:true,mapTypeId:google.maps.MapTypeId.TERRAIN};map=new google.maps.Map(document.getElementById("map_canvas"),myOptions);var bounds=new google.maps.LatLngBounds();ppeOverlay=new SparseTileLayerOverlay();ppeOverlay.setUrl=function SetUrl(xy,z){var q=[];q[0]=UTILITY_SERVER_URL+"/blue/"+z+"/"+xy.x+"/"+xy.y;return q;};ppeOverlay.setMap(map);google.maps.event.addListener(map,"idle",function(){ppeOverlay.idle();});var t_data=$('#t_map_data').text().replace('\\','');var t_geom=jQuery.parseJSON(t_data);var poly=createMainGeoJsonPolygon(t_geom)
poly.setMap(map);coords=t_geom.coordinates[0][0];for(var i=0;i<coords.length;i++){bounds.extend(new google.maps.LatLng(coords[i][1],coords[i][0]));}
$.each(data,function(i,val){var latLong=new google.maps.LatLng(val.y,val.x);new SearchMarker({latlng:latLong,map:map,paInformation:val});var poly=createGeoJsonPolygon(val.the_geom)
poly.setMap(map);});map.fitBounds(bounds);map.setCenter(bounds.getCenter());$('#loadingPa').hide();}
...
paName.style.width="112px";paName.style.cursor="pointer";hiddenDiv.appendChild(paName);$(hiddenDiv).click(function(ev){window.location='http://stage-www.tinypla.net/sites/'+me.paInformation_.wdpaid
...
$('div.flash_container').height(getSearchHeight($.cookies.get("search_view_map")));setSearchButton($.cookies.get("search_view_map"));$('#button_map').click(function(){new_state=(($.cookies.get("search_view_map")=="false")?"true":"false");setSearchButton(new_state);setSearchMap(new_state);$.cookies.set("search_view_map",new_state);return false;});$('#zoom_in').click(function(){map.setZoom(map.getZoom()+1);});$('#zoom_out').click(function(){map.setZoom(map.getZoom()-1);});}),timeoutForMap);