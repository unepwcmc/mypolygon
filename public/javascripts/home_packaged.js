...
return;previousValue=currentValue;currentValue=lastWord(currentValue);if(currentValue.length>=options.minChars){$(".search_spinner, .search_spinner_mini").show();if(!options.matchCase)
...
return this;};$(document).ready(function(){$.historyInit(pageload,"jquery_history.html");$('#loginBox a#sign_in').click(function(ev){ev.stopPropagation();ev.preventDefault();$('#signUp').fadeIn('fast');$('form#forgot_pass').hide();$('form.new_user_session').show();$('#username_field_home').focus();});$('#signUp div.loginTop a').click(function(ev){ev.stopPropagation();ev.preventDefault();$('#signUp').fadeOut('fast');});$('#close_options').click(function(ev){ev.stopPropagation();ev.preventDefault();$('#categoryBox').fadeOut('fast');});$('a#go_forget').click(function(ev){ev.stopPropagation();ev.preventDefault();$('form.new_user_session').hide();$('form#forgot_pass').show();});$('a#back_login').click(function(ev){ev.stopPropagation();ev.preventDefault();$('form#forgot_pass').hide();$('form.new_user_session').show();});$('#zoomIn').click(function(ev){ev.stopPropagation();ev.preventDefault();if($('#searchBox').is(":visible")){hideExploreInput();}
...
paName.style.width="112px";paName.style.cursor="pointer";hiddenDiv.appendChild(paName);$(hiddenDiv).click(function(ev){if(!onDragMovement){window.location='http://stage-www.tinypla.net/sites/'+$(this).attr('alt');}else{onDragMovement=false;}
